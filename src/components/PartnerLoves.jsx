  ...
  return (
    <div className="mx-w-125">
      <div className="text-center pt-5 px-5">
        <h1 className="text-3xl md:text-4xl lg:text-[40px] font-semibold text-[#000000]"><span className="text-[#06B8FF]">Partners</span> who Loves us</h1>
     ...
     <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-y-7 justify-items-center py-12 px-12 lg:px-20">
        {partnerBox.map((item)=>{
          return(
            <div key={item.id} className="w-48 h-28 md:w-59 md:h-37 flex items-center justify-center bg-white shadow-sm rounded-xl mx-5">
              <img className="w-36 h-24 md:w-45 md:h-34" src={item.image} alt="" />
            </div>
          )
        })}
      </div>
